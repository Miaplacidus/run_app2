  ...
  function submit_post_filters_form(){
    ...
    $.ajax({
      ...
      success : function(json_posts) {
        ...
        var post_info = "<li class='post_box'><ul class='post-summary'><li><div id='gmaps-container<%-post.id%>' style='position:relative;width:100%;height:220px;background-color:#e7eaf0;border-top-left-radius:7px;border-top-right-radius:7px;'></div><div id='zoom-in'></div><div id='zoom-out'></div></li> <li><h3><%- post.address %></h3></li> <li><div class='post-summary-contents'><%- post.time_in_tz %></div></li> <li><div class='post-summary-contents'><%- post.gender_preference %></div></li> <li><div class='post-summary-contents'>Pace: <%- post.pace_title %></div></li> <li><div class='post-summary-contents'>Age: <%- post.age_preference_range %></div></li> <li><div class='post-summary-contents'>Minimum Distance: <%- post.min_distance %></div></li> <li><div class='post-summary-contents'>Commitment: $<%- post.min_amt %></div></li> <li><div class= 'post-summary-contents'>Notes: <%- post.notes %></div></li> <li><div class='post-summary-contents'>Associated Circle: <% if (post.circle) { %><%- post.circle.name %> <% } %></div></li> <li><div class='post-summary-contents'>Organizer: <%- post.organizer.first_name %>, <%- post.organizer.gender %></div></li> <li><div class='post-summary-contents'>Runners: <%- post.runners %> -> <%- post.runners.count %> / <%- post.max_runners %></div></li></ul></li>";
